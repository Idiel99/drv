// ============================================================================
// DRV Development LLC — shared UI primitives
// ============================================================================
const { useState, useEffect, useRef, useMemo, useCallback, createContext, useContext } = React;
const { motion, AnimatePresence, useScroll, useTransform, useMotionValue, useSpring, useInView } = window.framerMotion || {};

// ---- Language context ------------------------------------------------------
const LangCtx = createContext({ lang: "en", t: window.COPY.en, setLang: () => {} });
...
// ---- Magnetic button -------------------------------------------------------
function MagneticBtn({ children, onClick, variant = "primary", className = "", as = "button", href }) {
  ...
  return (
    ...
    >
      ...
      <svg className="relative z-10 w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M5 12h14M13 5l7 7-7 7" strokeLinecap="round" strokeLinejoin="round" /></svg>
    </Comp>
  );
}

// ---- Reveal on scroll (split text or block) --------------------------------
function Reveal({ children, delay = 0, className = "", y = 24, as = "div" }) {
...
function CountUp({ to, suffix = "", duration = 1.6 }) {
  ...
  useEffect(() => {
    ...
    // Start after slight delay so the number reads as "counting up"
    const startDelay = setTimeout(() => { raf = requestAnimationFrame(step); }, 250);
    // Safety net: if animation never finishes (e.g. tab backgrounded), force-set final value
...
// ---- Architectural placeholder image --------------------------------------
function ArchPlaceholder({ tone = "warm", h = 360, label, className = "" }) {
  // Stripey architectural placeholder with monospace label
  const palettes = {
...
window.DRV_UI = { LangProvider, LangCtx, useLang, MagneticBtn, Reveal, SplitHeadline, CountUp, Eyebrow, ArchPlaceholder };
